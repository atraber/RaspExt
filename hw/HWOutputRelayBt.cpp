...
HWOutputRelayBt::HWOutputRelayBt()
{
    m_slaveAddress = -1;
    m_channel = -1;
    m_btThread = NULL;
...
HWOutput* HWOutputRelayBt::load(QDomElement *root)
{
    HWOutputRelayBt* hw = new HWOutputRelayBt();
...
QDomElement HWOutputRelayBt::save(QDomElement* root, QDomDocument* document)
{
...
void HWOutputRelayBt::setI2CBt(BTThread* btThread)
{
    // allocate our buffer
    BTI2CPacket packet;
    ...
    packet.commandBuffer[1] = m_value ? 0xFF : 0x000;
...
void HWOutputRelayBt::setupI2CBt(BTThread* btThread)
{
    // allocate our buffer
...
void HWOutputRelayBt::init(ConfigManager *config)
{
    ...
    // setup I2C device
    m_btThread->addOutput(std::bind(&HWOutputRelayBt::setupI2CBt, this, std::placeholders::_1));
}

void HWOutputRelayBt::deinit(ConfigManager* config)
{
    m_btThread = NULL;
}

void HWOutputRelayBt::outputChanged()
{
    HWOutputRelay::outputChanged();

    if(m_btThread != NULL)
        m_btThread->addOutput(std::bind(&HWOutputRelayBt::setI2CBt, this, std::placeholders::_1));
}
